	...
	try {
		for (int i = 0; i < lines.size(); i++) {
			...
			if (lineData.length() != 0 && lineData != "" && lineData != NULL) {
				...
				for (int j = 0; j < splitedLineData.size(); j++) {
				...
				if (getEntity() == 'c') {
					...
					Party party = Party(splitedLineData[3]);
					party.setNumber(
						stoi(splitedLineData[0])
					);
					party.setPartyVotes(
...
vector<Candidate*> Election::getCandidatesOrderedByVotes() {
	...
	sort(allCandidates.begin(), allCandidates.end(), CandidatesVotesComparator());
...
vector<Party*> Election::getPartiesOrderedByTotalVotes() {
	...
	sort(partiesOrderedByTotalVotes.begin(), partiesOrderedByTotalVotes.end(), PartiesTotalVotesComparator());
...
float Election::getGeneralPercent(float p, float t) {
	float r = 0.00f;
...
vector<Party*> Election::getPartiesOrderedByPartyVotes() {
	...
	sort(partiesOrderedByPartyVotes.begin(), partiesOrderedByPartyVotes.end(), PartiesPartyVotesComparator());
...
vector<Party*> Election::getPartiesWithCandidatesOrderedByVotes() {
	...
	for (Party party : parties) {
		...
		sort(candidates.begin(), candidates.end(), CandidatesVotesComparator());
	}
	sort(parties.begin(), parties.end(), PartiesCandidateVotesComparator());
